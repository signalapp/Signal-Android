 ...
 */
internal class LicensesPlugin : Plugin<Project> {
  override fun apply(target: Project) {
    val out = target.serviceOf<StyledTextOutputFactory>().create("output")

    target.tasks.register("saveLicenses") {
      ...
      doLast {
        ...
        val resolvedDependencies: List<ResolvedDependency> = target.configurations
          .asSequence()
          ...
          .flatten()
          .distinctBy { it.file }
          .map { pomDependency(target, it.id.componentIdentifier.toString()) }
          .mapNotNull { it.toResolvedDependency(target, out) }
        ...
        for (resolvedDependency in resolvedDependencies) {
          for (license in resolvedDependency.licenses) {
            ...
            if (licenseData == null) {
              printlnError(out, "Failed to find matching license data for ${license.name} (${license.url}), which is in use by $resolvedDependency")
 ...
 */
private fun Dependency.toResolvedDependency(project: Project, out: StyledTextOutput, leafDependency: ResolvedDependency? = null): ResolvedDependency? {
  ...
  val pomFile: File = try {
    ...
    printlnWarning(out, "[${this.id}] Failed to resolve the POM dependency to a file.")
    return null
  }
  ...
  val licenses: List<RawLicense> = try {
    ...
    printlnWarning(out, "[${this.id}] Error when parsing XML")
    e.printStackTrace()
    emptyList()
  }
  ...
  if (resolvedDependency.licenses.isEmpty()) {
    ...
    if (parentGroup != null && parentName != null && parentVersion != null) {
      printlnNormal(out, "[${this.id}] Could not find a license on this node. Checking the parent.")
      return pomDependency(project, "$parentGroup:$parentName:$parentVersion").toResolvedDependency(project, out, leafDependency = resolvedDependency)
    }
  } else if (leafDependency != null) {
    printlnNormal(out, "[${leafDependency.id}] Found a license on a parent dependency. (parent = ${this.id})")
  }

  return resolvedDependency
}

private fun printlnNormal(out: StyledTextOutput, message: String) {
  out.style(StyledTextOutput.Style.Normal).println(message)
}

private fun printlnWarning(out: StyledTextOutput, message: String) {
  out.style(StyledTextOutput.Style.Description).println(message)
}

private fun printlnSuccess(out: StyledTextOutput, message: String) {
  out.style(StyledTextOutput.Style.SuccessHeader).println(message)
}

private fun printlnError(out: StyledTextOutput, message: String) {
  out.style(StyledTextOutput.Style.FailureHeader).println(message)
}

private fun pomDependency(project: Project, locator: String): Dependency {
  return project.dependencies.create("$locator@pom")
}

private fun Configuration.tryResolveConfiguration(): ResolvedConfiguration? {
  return try {
...
private fun ResolvedConfiguration.tryToResolveArtifacts(): Set<ResolvedArtifact>? {
  return try {
...
private fun GPathResult.get(key: String): GPathResult? {
...
private data class RawLicense(val name: String, val url: String)
