...
public final class PushProcessMessageJob extends BaseJob {
    ...
    @Nullable
    private final byte[] serializedPlaintextContent;
    @Nullable
        ...
        try {
            ...
            if (content.getDataMessage().isPresent()) {
                ...
                if (isInvalidMessage(message))
                    handleInvalidMessage(content.getSender(), content.getSenderDevice(), toEncodedId(message.getGroupInfo()), content.getTimestamp(), smsMessageId);
                ...
                if (syncMessage.getSent().isPresent())
                    ...
                    handleSynchronizeStickerPackOperation(syncMessage.getStickerPackOperations().get());
                else if (syncMessage.getConfiguration().isPresent())
                    handleSynchronizeConfigurationMessage(syncMessage.getConfiguration().get());
                    ...
                    handleSynchronizeFetchMessage(syncMessage.getFetchType().get());
    ...
    Optional<String> toEncodedId(@NonNull Optional<SignalServiceGroup> groupInfo) {
...
//    }
    ...
    public static final class Factory implements Job.Factory<PushProcessMessageJob> {
        @Override
        public @NonNull
        PushProcessMessageJob create(@NonNull Parameters parameters, @NonNull Data data) {
            try {
