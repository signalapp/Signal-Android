  ...
  protected void onRun() throws Exception {
    ...
    if (distributionId != null) {
      ...
      if (!groupRecord.isPresent()) {
        Log.w(KEY, "Could not find a matching group for the distributionId! Skipping message send.");
        return;
      } else if (!groupRecord.get().getMembers().contains(recipientId)) {
        Log.w(KEY, "The target user is no longer in the group! Skipping message send.");
        return;
      }
