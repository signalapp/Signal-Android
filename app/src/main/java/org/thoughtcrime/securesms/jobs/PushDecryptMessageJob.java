...
public final class PushDecryptMessageJob extends BaseJob {
    ...
    public PushDecryptMessageJob(Context context, long pushMessageId, long smsMessageId) {
        this(new Parameters.Builder().setQueue(QUEUE).setMaxAttempts(Parameters.UNLIMITED).build(),
                pushMessageId,
                smsMessageId);
        setContext(context);
    }
    ...
    private @NonNull
    List<Job> handleMessage(@NonNull SignalServiceEnvelope envelope) throws NoSenderException {
        try {
    ...
    public static final class Factory implements Job.Factory<PushDecryptMessageJob> {
        @Override
        public @NonNull
        PushDecryptMessageJob create(@NonNull Parameters parameters, @NonNull Data data) {
