...
object NotificationFactory {
  ...
  private fun notify24(
    ...
    state.conversations.forEach { conversation ->
      if (conversation.threadId == visibleThreadId && conversation.hasNewNotifications()) {
        ...
        notifylauncher3(context = context, state = state.copy(conversations = state.getNonVisibleConversation(visibleThreadId)))
        notifyForConversation(
          context = context,
          conversation = conversation,
          targetThreadId = targetThreadId,
  ...
  private fun notifylauncher3(context: Context, state: NotificationStateV2) {
    val intent = Intent()
