...
public class ThreadDatabase extends Database {
  ...
  private PduPart getPartForMessageRecordSnippet(MessageRecord record) {
    if (record.isMms() && !record.isMmsNotification()) {
      ...
      for (Pair<Long, PduPart> partPair : recordParts) {
        if (MediaUtil.isImage(partPair.second) ||
            MediaUtil.isAudio(partPair.second) ||
            MediaUtil.isVideo(partPair.second))
        {
          return partPair.second;
        }
      }
    }
  ...
  public class Reader {
    ...
    private boolean snippetPartIsImage(long snippetType, String snippetPartType) {
      if (snippetType == MmsSmsColumns.Types.BASE_DRAFT_TYPE) {
        return snippetPartType.equals(DraftDatabase.Draft.IMAGE);
      }
    ...
    {
      if (snippetPartUri  == null ||
          snippetPartType == null ||
          !snippetPartIsImage(snippetType, snippetPartType))
      {
        return null;
      }
