...
import org.signal.core.util.ListenableFutureTask
import org.signal.core.util.SoftHashMap
import org.signal.core.util.ThreadUtil
import org.signal.core.util.concurrent.SignalExecutors
import org.signal.core.util.concurrent.SimpleTask
import org.signal.core.util.logging.Log
import org.signal.emoji.parsing.EmojiDrawInfo
import org.signal.emoji.protos.JumbomojiPack
...
object JumboEmoji {
  ...
  fun updateCurrentVersion(context: Context) {
    SignalExecutors.BOUNDED.execute {
      ...
      if (EmojiFiles.getLatestEmojiData(context, version)?.format != null) {
        ...
        ThreadUtil.runOnMain { downloadedJumbos.addAll(EmojiDependencies.getDownloadedJumboSheets(version.version)) }
      }
    }
  }
  ...
  @JvmStatic
  @MainThread
  fun canDownloadJumbo(): Boolean {
    val now = SystemClock.elapsedRealtime()
    if (now - networkCheckThrottle > lastNetworkCheck) {
      canDownload = EmojiDependencies.canAutoDownloadJumboEmoji()
  ...
  fun loadJumboEmoji(context: Context, drawInfo: EmojiDrawInfo): LoadResult {
    ...
    val newTask = ListenableFutureTask<Bitmap> {
      ...
      if (uuid == null) {
        if (!EmojiDependencies.canAutoDownloadJumboEmoji()) {
        ...
        EmojiDependencies.onJumboSheetDownloaded(version.version, drawInfo.jumboSheet!!)
      }
