...
class EmojiSource(
  ...
  companion object {
    ...
    private fun loadRemoteBasedEmojis(): EmojiSource? {
      if (EmojiDependencies.forceBuiltInEmoji) {
        return null
      }

      val context = EmojiDependencies.application
    ...
    fun loadAssetBasedEmojis(): EmojiSource {
      val emojiData: InputStream = EmojiDependencies.application.assets.open("emoji/emoji_data.json")
