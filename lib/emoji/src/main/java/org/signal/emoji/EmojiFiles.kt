...
private fun getFilesUri(name: String, format: String): Uri = PartAuthorityUris.getEmojiUri(name)

private fun getOutputStream(context: Context, outputFile: File): OutputStream {
  val attachmentSecret = AttachmentSecretProvider.getInstance(context, EmojiDependencies.attachmentSecretStore).orCreateAttachmentSecret
